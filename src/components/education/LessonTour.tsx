...
export default function LessonTour({ copy, total, active, isLast, gateBlocked, gateHint, ctaLabel, onNext, onCta }: LessonTourProps) {
  return (
    <div className="border-t border-border pt-6">
      ...
      {isLast ? (
        <button onClick={onCta} className="mt-3 w-fit rounded-full bg-ink px-6 py-3 font-sans text-sm font-semibold text-fg-inverse">
          {ctaLabel}
        </button>
      ) : (
        <button
          ...
          className={`mt-3 w-fit rounded-full px-6 py-3 font-sans text-sm font-semibold ${gateBlocked ? "bg-surface-sunken text-fg-muted" : "bg-ink text-fg-inverse"}`}
        >
