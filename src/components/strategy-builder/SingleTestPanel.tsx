...
const SingleTestPanel = forwardRef<
  SingleTestHandle,
  { initialDraft?: StrategyDraft; initialSymbol?: string; pickerSlot?: React.ReactNode }
>(function SingleTestPanel({ initialDraft, initialSymbol, pickerSlot }, ref) {
  const { user } = useAuth();
  ...
  const loadDraft: SingleTestHandle["loadDraft"] = (d, opts) => {
    setDraft(d);
    setActiveSetupTile(deriveSetupTileKey(d));
    setActiveRecipe(null);
  ...
  useEffect(() => {
    if (initialDraft) loadDraft(initialDraft, { symbol: initialSymbol });
